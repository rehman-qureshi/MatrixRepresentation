...
def add_true_exclusion_relationships_function(df):
    ...
    for activity in df.index:
        for other_activity in df.columns:
            if not (df.at[activity, other_activity] == '→' or df.at[activity, other_activity] == '~>' or df.at[activity, other_activity] == '||' or activity == other_activity):
                    # Check if a path exists from activity to other_activity
                    #print(f"Checking true exclusion relationship between {activity} and {other_activity}")
                    if not path_exists(df, activity, other_activity):   # check if there is no path from activity to other_activity             
                        if not path_exists(df, other_activity,activity): # also check if there is no path from other_activity to activity
                            ...
                            #print(f"Added true exclusion relationship: {activity} # {other_activity}")
